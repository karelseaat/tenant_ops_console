    ...
    <main class="shell">
        ...
        <section class="stats">
            @foreach ($tenantStatuses as $status)
                <article class="stat">
                    <span>{{ $status }} tenants</span>
                    <strong>{{ $tenantStatusCounts[$status] ?? 0 }}</strong>
                </article>
            @endforeach
            <article class="stat">
                <span>Healthy checks</span>
                <strong>{{ $checkStatusCounts['ok'] ?? 0 }}</strong>
            </article>
            <article class="stat">
                <span>Warnings</span>
                <strong>{{ $checkStatusCounts['warn'] ?? 0 }}</strong>
            </article>
            <article class="stat">
                <span>Failures</span>
                <strong>{{ $checkStatusCounts['fail'] ?? 0 }}</strong>
            </article>
        </section>
                ...
                @foreach ($tenants as $tenant)
                    <article class="tenant">
                        <div class="top">
                            <div>
                                <h3>{{ $tenant->name }}</h3>
                                <div class="muted">{{ $tenant->plan_name }} plan · owner {{ $tenant->owner_name }}</div>
                            </div>
                            <span class="pill" data-status="{{ $tenant->status }}">{{ $tenant->status }}</span>
                        </div>

                        <div class="meta">
                            <span>Region {{ $tenant->region }}</span>
                            <span>{{ $tenant->seat_count }} seats</span>
                            <span>Renewal {{ optional($tenant->renewal_at)->format('Y-m-d') ?? 'n/a' }}</span>
                            <span>Last incident {{ optional($tenant->last_incident_at)->diffForHumans() ?? 'none' }}</span>
                        </div>

                        <ul>
                            @foreach ($tenant->serviceChecks as $check)
                                <li>
                                    <strong>{{ $check->service_name }}</strong>
                                    <span class="pill" data-status="{{ $check->status }}">{{ $check->status }}</span>
            ...
            <aside class="panel">
                ...
                @foreach ($alerts as $alert)
                    <article class="tenant">
                        <div class="top">
                            <div>
                                <h3>{{ $alert->tenant?->name }}</h3>
                                <div class="muted">{{ $alert->service_name }}</div>
                            </div>
                            <span class="pill" data-status="{{ $alert->status }}">{{ $alert->status }}</span>
                        </div>
                        <div class="meta">
                            <span>{{ $alert->checked_at?->format('Y-m-d H:i') }}</span>
                        </div>
